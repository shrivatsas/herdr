...
### Added
- Herdr now identifies Omnigent sessions launched via their Python-wrapped `omni`/`omnigent` entry points as a first-class Omnigent agent kind, surfacing them in the agent list, sidebar, and `herdr agent explain`, with idle/working screen detection from its status footer.
